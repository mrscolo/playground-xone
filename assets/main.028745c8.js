...
`;window.MonacoEnvironment={getWorker:function(e,t){switch(t){case"css":return new K;case"javascript":return new W;default:return new H}}};const de=()=>{var t;return{automaticLayout:!0,fontSize:Number((t=localStorage.getItem("fontSize"))!=null?t:"14"),scrollBeyondLastLine:!0,padding:{top:16},minimap:{enabled:localStorage.getItem("minimap")!=="off"},theme:localStorage.getItem("theme")||"vs-dark"}},V=new DOMParser().parseFromString(re,"text/xml"),E={version:1.1,properties:[]};V.querySelectorAll("attribute").forEach(e=>{const t=e.getAttribute("name");E.properties.some(a=>a.name===t)||E.properties.push({name:t})});x.css.cssDefaults.setOptions({data:{useDefaultDataProvider:!1,dataProviders:[E]}});x.typescript.javascriptDefaults.addExtraLib(`
	${le}
	${ue}
	${xe}
	${me}
